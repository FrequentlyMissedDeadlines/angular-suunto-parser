describe('DM5 XML Parser', function() {
    ...
    var sampleXmlFile = blobToFile(new Blob([sampleXml], {
        type: "text/xml",
    }), "testFile.txt");
    var sampleXmlSingleSampleFile = blobToFile(new Blob([sampleXmlSingleSample], {
        type: "text/xml",
    }), "testFile.txt");
    ...
    it('Should parse a simple file', function(done) {
        ...
        DM5Parser.parseFiles([sampleXmlFile], scope).then(function(result) {
            ...
            expect(result[0].Dive.PressureBlob['_i:nil']).toEqual(false);
            expect(result[0].Dive.Algorithm['_i:nil']).toEqual(true);
            done();
        });
